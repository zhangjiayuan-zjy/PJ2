...
function getData(page){
    ...
    $.post("../functionPHP/filter_myPhotos.php",{
        pageSize:pageSize,
        startIndex: page-1
    },function (data) {
        total=data["total"];
        ...
        currentPage=page;
        if (totalPage==0){
            let tipSpan=$("<span>您还没有上传图片</span>");
            $(".helptips").empty();
            $(".helptips").append(tipSpan);
        }else {
            $(".main").empty();
            for (let i = 0; i < data["result"].length; i++) {
                let div = $("<div class='detail'><div class='image'><a><img src='../../../img/normal/small/" + data["result"][i]["PATH"] + "' class='filterImg' data-path='" + data["result"][i]["PATH"] + "' data-title='" + data["result"][i]["Title"] + "' data-description='" + data["result"][i]["Description"] + "'></a></div><div class='dp'><h2>"+data["result"][i]["Title"]+"</h2><div class='imagedp'><p>"+data["result"][i]["Description"]+"</p></div><div class='input'><input type='button'  value='Modify' class='modify'> <input type='button' class='delete' value='delete'></div></div></div>");
                $(".main").append(div);
            }
            if (totalPage > 5) {
                totalPage = 5;
...
$(".main").on("click",function (event) {
    ...
    }else if ($(event.target).val()=="Modify"){
        let img=$(event.target).parents(".detail").find(".image a img");
        $(location).attr("href","modifyPhoto.php?path="+img.attr("data-path")+"&title="+img.attr("data-title")+"&des="+img.attr("data-description"));
